/*
  ...
  - Added the required column `dayId` to the `days_habits` table without a default value. This is not possible if the table is not empty.
  - Added the required column `habitId` to the `days_habits` table without a default value. This is not possible if the table is not empty.
...
CREATE TABLE "new_days_habits" (
    ...
    CONSTRAINT "days_habits_habit_id_fkey" FOREIGN KEY ("habit_id") REFERENCES "habts" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE "new_habit_week_days" (
    ...
    "habitId" TEXT NOT NULL,
    CONSTRAINT "habit_week_days_habit_id_fkey" FOREIGN KEY ("habit_id") REFERENCES "habts" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
